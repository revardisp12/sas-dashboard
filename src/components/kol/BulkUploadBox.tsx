...
export function BulkUploadBox({ columns, children }: { columns: string; children?: ReactNode }) {
  return (
    <div style={{ border: '2px dashed #E5E7EB', borderRadius: 12, padding: 18, background: '#F8FAFC', marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        ...
        <button style={{ fontSize: 12, fontWeight: 600, padding: '7px 12px', borderRadius: 8, background: '#fff', color: '#0EA5E9', border: '1px solid #0EA5E9', cursor: 'pointer', whiteSpace: 'nowrap' }}>
          ⬇ Download template
        </button>
      </div>
      {children}
