...
import { supabase, type UserProfile, type UserRole, type UserBrand } from '@/lib/supabase'
...
export default function UserManagement({ brandColor }: Props) {
  ...
  async function saveEdit(id: string) {
    ...
    const { error } = await supabase
      .from('user_profiles')
      .update({ role: editRole, brand: brandValue })
      .eq('id', id)
  ...
  return (
    <div className="space-y-4">
      ...
      {error && (
        <div className="text-xs rounded-lg px-3 py-2" style={{ background: '#FEF2F2', color: '#DC2626', border: '1px solid #FECACA' }}>
          {error}
        </div>
      )}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {users.map((u, i) => {
              ...
              return (
                <tr key={u.id}
                  style={{ borderBottom: i < users.length - 1 ? '1px solid #F3F4F6' : 'none', background: isEditing ? `${brandColor}05` : '#fff' }}>
                  <td className="px-4 py-3">
                    <span className="font-medium" style={{ color: '#111827' }}>{u.full_name || '(no name)'}</span>
                    {isMe && <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded-full" style={{ background: `${brandColor}20`, color: brandColor }}>Kamu</span>}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {isEditing ? (
                      editRole === 'super_admin' ? (
                        ...
                        <select
                          value={editBrand ?? ''}
                          onChange={e => setEditBrand(e.target.value as UserBrand || null)}
                          className="text-xs px-2 py-1 rounded border outline-none"
                          style={{ borderColor: brandColor, color: '#374151' }}
                        >
      ...
      <p className="text-[11px]" style={{ color: '#9CA3AF' }}>
        Untuk invite user baru, kirimkan link Supabase Auth ke admin dan minta mereka daftar lewat email.
      </p>
    </div>
  )
}
